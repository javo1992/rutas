...
class rutasC
{
	...
	{
		$sensor = $this->modelo->buscar_contenedores($estado=false,$id=false,$query=false,$codigo=$id);
		$datos[0]['campo'] ='estado';
		$datos[0]['dato'] =$es;
		...
		$datosH[0]['campo'] ='estado';
		$datosH[0]['dato'] =$es;
		$datosH[1]['campo'] ='sensor';
		$datosH[1]['dato'] =$sensor[0]['id'];
		$datosH[2]['campo'] ='fecha';
		$datosH[2]['dato'] =date('Y-m-d');

		$this->modelo->add('historial',$datosH);
		return $this->modelo->update('contenedores',$datos,$where);
